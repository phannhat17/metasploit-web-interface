...
@scanres.route('/<ip>/<timestamp>', methods=['GET'])
def serve_filtered_data(timestamp, ip):
    """
    Return the entire JSON data for a given timestamp and IP address.
    """
    data = load_json_file(timestamp, ip)
...
@scanres.route('/run-stats/<ip>/<timestamp>', methods=['GET'])
def runstats(timestamp, ip):
    """
    Return the "TimeStr" and "Exit" fields from the "Finished" section of the JSON data.
    """
    data = load_json_file(timestamp, ip).get("runstats")
    if data:
        finished_data = data.get('finished')
        if finished_data:
            time_str = finished_data.get('@timestr')
            exit_status = finished_data.get('@exit')
            ...
            return jsonify({"error": "Finished section not found in the JSON data"}), 404
    else:
        return jsonify({"error": f"Not found the scan result for {timestamp} at {ip}"}), 404
 
@scanres.route('/port-result/<ip>/<timestamp>', methods=['GET'])
def portres(timestamp, ip):
    """
    Return the "Port" information under "Host" in the JSON data.
    """
    data = load_json_file(timestamp, ip)
    if data:
        host_data = data.get('host')
        if host_data and 'ports' in host_data:
            return jsonify({"Port": host_data['ports']['port']})
        else:
            return jsonify({"error": "Port key not found under Host in the JSON data"}), 404
    else:
        return jsonify({"error": f"Not found the scan result for {timestamp} at {ip}"}), 404
